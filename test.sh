...
if [ -d "./checker" ]; then
    echo "Running checker in ./checker"
        cd ./checker && flagdata=$(docker run -v ./:/test glitchrange/checkertest | tee /dev/tty | grep "flag")  && cd ..
...
for dir in ./exploits/*; do
    if [ -d "$dir" ]; then
        ...
        if [ -f "exploit.py" ]; then
            exploit_output=$(docker run -v ./:/exploit python:latest bash -c "cd /exploit; pip install --no-cache-dir --root-user-action=ignore --disable-pip-version-check --quiet -r requirements.txt; python exploit.py host.docker.internal '$flag_id'" | tee /dev/tty)
